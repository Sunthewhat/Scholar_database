...
const EditFormPage: FC = () => {
	...
	// Use useApiData for scholar fields
	const {
	...
	// Initialize sections when data is loaded
	useEffect(() => {
		if (initialSections) {
	...
	useEffect(() => {
		if (isInitialError) {
			setSaveError('เกิดข้อผิดพลาดในการโหลดข้อมูลฟอร์ม');
		}
	}, [isInitialError]);
	...
	const handleSave = async () => {
		...
		try {
			// For existing sections, update them. For new sections, create them.
			const updatePromises = sections.map(async (section) => {
				...
				if (section._id) {
					...
					const response = await Axios.put<ScholarFieldResponse.update>(
						`/scholar-field/${section._id}`,
						{
							field_name: sectionData.field_name,
							field_label: sectionData.field_label,
							field_description: sectionData.field_description,
					...
					if (response.status !== 200 || !response.data.success) {
						throw new Error(response.data.msg || 'เกิดข้อผิดพลาดในการอัปเดต');
					}

					return response.data.data;
				} else {
					// Create new section
					const response = await Axios.post<ScholarFieldResponse.create>(
						'/scholar-field',
						sectionData
					);

					if (response.status !== 200 || !response.data.success) {
						throw new Error(response.data.msg || 'เกิดข้อผิดพลาดในการสร้าง');
					}
				...
				<div className='w-3/4 h-full flex flex-col mx-auto pt-16 mt-20'>
					...
					<div className='w-full h-10 flex justify-between'>
						<div className='flex gap-4'>
							...
							>
								<h1>{isSaving ? 'กำลังบันทึก...' : 'บันทึกการแก้ไข'}</h1>
							</button>
							<button
								onClick={() => router.back()}
								className='w-32 bg-red text-center text-white px-6 py-2 rounded-xl'
							>
								ยกเลิก
							</button>
						</div>
